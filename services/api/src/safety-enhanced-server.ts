...
const safetyViolations = new Map(); // Track safety violations per user
...
// ===== M2 + M3 + M4: SESSIONS & MESSAGES WITH AI & SAFETY =====
...
fastify.post('/sessions/:id/messages', async (request, reply) => {
  ...
  const { validateContentSafety } = await import('./safety/boundary-detector');
  ...
  const message = {
    ...
    createdAt: new Date(),
    safetyTags: request.safetyContext?.boundaryResult?.concerns || [],
...
async function start() {
  try {
    ...
    console.log(`🚀 Sync API server (M1+M2+M3+M4) running on http://${host}:${port}`);
    ...
    console.log(`🧪 Ready for testing! Use the test client: node test-m4-safety.js`);
  } catch (err) {
