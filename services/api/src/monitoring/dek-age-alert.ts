...
export async function registerDEKAgeMonitoring(fastify: FastifyInstance) {
  ...
  fastify.get('/health/dek-age', async (request, reply) => {
    try {
      ...
      fastify.log.error('Failed to get DEK age status', { error: error instanceof Error ? error.message : "Unknown error" });
      reply.code(500).send({
  ...
  fastify.post('/admin/force-dek-alert', async (request, reply) => {
    try {
      ...
      fastify.log.error('Failed to force DEK age alert', { error: error instanceof Error ? error.message : "Unknown error" });
      reply.code(500).send({
        error: 'Internal Server Error',
  ...
  setInterval(async () => {
    try {
      ...
      fastify.log.error('DEK age monitoring error', { error: error instanceof Error ? error.message : "Unknown error" });
    }
  }, checkInterval);
